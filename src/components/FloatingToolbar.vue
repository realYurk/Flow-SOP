...
function strip<T>(obj: T): T { return JSON.parse(JSON.stringify(obj)) }
...
async function doImport(filePath: string) {
  ...
  try {
    ...
    if (d) { store.data = d; await store.saveData(); store.toast('✓ 数据导入成功！') }
    else     store.toast('❌ 导入失败，文件格式有误', 'error')
  } catch(e) { store.toast('❌ 出错: ' + String(e), 'error') }
}
</script>
